...
| A04 | Live interaction preview and one durable command per completed gesture | Verified | Camera and splitter total-displacement/out-and-back tests; no-op history defence; coalesced final-move/release egui tests; focus/terminal-pointer cancellation tests; exact linked-camera and splitter history tests; browser/native physical drag, undo/redo and no-op assertions; hidden-origin, cross-linked, unlinked and wheel-to-drag lifecycle tests; native polygon/edit/undo/redo sequence | Raster, overlays, coordinates, overview and demands share one preview camera; camera and splitter drags include release-edge movement, exact origin return creates no command, abnormal camera termination cancels, and wheel bursts coalesce by link scope after 140 ms. |
...
| V02 | Native release actually launched and required interactions captured | Verified | `tools/native-smoke.sh`, `native-semantic.json`, runtime log, seven native screenshots | xdotool resolves current Rust-owned pane, tab, splitter, control, result and scroll rectangles through the live window; camera, gallery and splitter results—including hash-changing resize, exact undo/redo and no-op return—are asserted through Rust snapshots, and the failure scan rejects panic and fatal wgpu errors. |
| V03 | Browser Wasm actually launched in a real browser, checked and interacted with | Verified | Playwright readiness plus Rust-owned `browser-semantic.json`; semantic geometry targets; exact physical camera/splitter drag, wheel, undo/redo and out-and-back snapshots; six-sample splitter preview trace; zero-viewport/Worker-failure probes; compositor screenshots; console failure hooks; pane drag/save/reload; independent Lantern resize/canvas inspection | Physical input claims enter through browser events at rectangles reported by the current Rust frame; semantic actions remain for architecture/lifecycle invariants. |
...
| Lockfile identity | SHA-256 `355c9e4a85767afc4d3a968cc7a1aec9242509b70f2c8b9def9dc4e05dfcfa9f` |
...
| --- | ---: | ---: | --- |
| Initial loading | 0.7 ms | 22.7 ms | Thirteen samples; startup/first pipeline work dominates the tail. |
| Four visible GPU viewports while panning | 0.6 ms | 1.2 ms | A real 90×50 drag produced the exact linked transform and one command; undo restored both cameras. |
| Warmed idle | n/a | n/a | Frame counter stayed 255 → 255 over 700 ms; no deliberate continuous repaint. |
...
| Million-row result scroll | 0.6 ms | 1.3 ms | The visible range plus eight explicit overscan rows was materialised. |
...
| Polygon editing | 0.4 ms | 0.7 ms | Three vertices and commit across linked panes. |
| Saved workspace restore | 0.7 ms | 3.7 ms | Persisted rearranged dock restored after a full reload. |
| Splitter interaction (additional) | 0.5 ms | 0.6 ms | Six preview samples tracked a 47-point drag; release produced one canonical split command and exact undo/redo restored both states. |
