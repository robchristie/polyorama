...
- latency p50/p95 values are real bounded-window percentiles, renderer preparation accumulates across jobs, callback passes/draws are counted when invoked, returned command buffers are truthfully zero, and application update CPU time is labelled as such;
- Rust-owned browser test actions and snapshots establish linked-camera, render-plan, command-history, annotation, canonical workspace, queue-bound, decoded-thumbnail and warmed-idle correctness. Fixed-coordinate interactions remain only for visual/runtime evidence.
...
| A02 | Egui is immediate presentation through narrow pane APIs | Verified | Feature-scoped `PaneSurface` fields and `panes/{results,thumbnails,inspector}.rs`; recursive architecture source scan | Pane code has no mutable complete app/session/runtime/workspace or wgpu/egui-wgpu access. |
...
| A04 | Live interaction preview and one durable command per completed gesture | Verified | Same-frame image-model test; exact linked-camera history tests; semantic drag/wheel history assertions; native polygon/edit/undo/redo sequence | Raster, overlays, coordinates, overview and demands share one preview camera; wheel bursts coalesce after 140 ms. |
...
| A07 | Typed render plan and correct logical/physical viewport, scale, clipping, focus and pointer-local mapping | Verified | Real `FrameOutput::render_plan`, opaque staged callback targets, central `submit_render_plan`, geometry tests and semantic render-camera snapshot | The shell finalises cross-pane preview cameras and demands, then publishes the complete plan before callback preparation. |
...
| F09 | Two-dimensional 100k thumbnail grid, bounded visible/overscan demand, placeholders, actual decoded content, stable selection and recenter path | Verified | Virtual-grid and bounded thumbnail-cache tests; semantic cache snapshot; browser/native gallery screenshots | Worker-decoded 64×64 scalar payloads are colour-mapped into a bounded 4 MiB presentation cache. |
...
| I01 | Live application-update/UI, workspace, measured renderer callbacks, scheduler/workers/cache/upload and virtualisation diagnostics | Verified | Diagnostics pane/screenshot and structured browser snapshot | GPU timestamp is explicitly `unavailable`; renderer returns zero command buffers because egui owns submission. |
...
| V03 | Browser Wasm actually launched in a real browser, checked and interacted with | Verified | Playwright readiness plus Rust-owned `browser-semantic.json`, compositor screenshots, console failure hooks, pan/zoom/split/drag/save/reload | Correctness uses semantic actions/snapshots; coordinates are retained only for visual interaction evidence. |
...
Summary: formatting passed; native and Wasm clippy passed with warnings denied; all 53 focused core/runtime/renderer/application tests passed; architecture boundaries passed; release native and both release Wasm packages built; Playwright browser smoke passed; native release smoke passed. The browser canvas was 1440×900 with eight registered panes, `wgpu-scalar` readiness, four GPU render jobs and completed Worker work. Rust-owned semantic snapshots proved exact linked cameras, render-plan camera equality, one-record gesture history, polygon commit/undo, dock-tree restoration, bounded queues, aborted dock-drag cleanup and decoded thumbnails. The responsive reload probes also produced non-zero 1280×720 and 900×700 canvases. Native smoke reported `GL/llvmpipe, 1440x900` and found no panic or fatal wgpu error.
...
| Lockfile identity | SHA-256 `030366477da914e3a43dc3c2b6ac5860b85aa135c92d0bacfca01f17ce0ac8b4` |
...
| --- | ---: | ---: | --- |
| Initial loading | 0.7 ms | 24.0 ms | Thirteen samples; startup/first pipeline work dominates the tail. |
| Four visible GPU viewports while panning | 0.7 ms | 0.9 ms | Semantic history confirms one completed drag command. |
| Warmed idle | n/a | n/a | Frame counter stayed 179 → 179 over 700 ms; no deliberate continuous repaint. |
| Rapid zoom/tile-level transitions | 0.8 ms | 0.9 ms | The 0.1.0 p95 baseline was 126.3 ms before worker-side production and desired-set scheduling. |
| Million-row result scroll | 0.8 ms | 1.2 ms | The visible range plus eight explicit overscan rows was materialised. |
| Thumbnail gallery scroll | 0.5 ms | 1.2 ms | Actual worker-decoded thumbnail pixels entered the bounded presentation cache. |
| Polygon editing | 0.4 ms | 0.6 ms | Three vertices and commit across linked panes. |
| Saved workspace restore | 0.7 ms | 3.6 ms | Persisted rearranged dock restored after a full reload. |
| Splitter interaction (additional) | 0.5 ms | 0.9 ms | A transient preview resolved to one canonical, undoable split command and recorded repaint reason. |
| Pane drag/drop (additional) | 0.5 ms | 0.7 ms | Derived View moved and the empty source split was pruned. |

Measured capability and counters from the restored browser snapshot include four GPU viewports/render jobs, four invoked callbacks/render passes, 20 actual draw calls, zero renderer-returned command buffers, a 64 MiB texture cache budget, 4 MiB upload budget, 655,360 resident texture bytes, zero in-flight decodes and zero failures. Dataset sizes are 1,000,000 results and 100,000 thumbnails.
