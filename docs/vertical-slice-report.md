...
| A04 | Live interaction preview and one durable command per completed gesture | Verified | `CameraDragSession` total-displacement and out-and-back tests; no-op history defence; coalesced final-move/release egui test; focus/terminal-pointer cancellation tests; exact linked-camera history tests; browser/native 90×50 physical drag and exact undo assertions; browser physical out-and-back assertion; hidden-origin, cross-linked, unlinked and wheel-to-drag lifecycle tests; native polygon/edit/undo/redo sequence | Raster, overlays, coordinates, overview and demands share one preview camera; the drag includes recognition and release-edge movement, exact origin return creates no command, abnormal termination cancels, and wheel bursts coalesce by link scope after 140 ms. |
...
| V02 | Native release actually launched and required interactions captured | Verified | `tools/native-smoke.sh`, `native-semantic.json`, runtime log, seven native screenshots | xdotool resolves current Rust-owned pane, tab, splitter, control, result and scroll rectangles through the live window; real drag/wheel results are asserted through Rust snapshots and the failure scan rejects panic and fatal wgpu errors. |
| V03 | Browser Wasm actually launched in a real browser, checked and interacted with | Verified | Playwright readiness plus Rust-owned `browser-semantic.json`; semantic geometry targets; exact physical drag/wheel/out-and-back snapshots; zero-viewport/Worker-failure probes; compositor screenshots; console failure hooks; split/drag/save/reload; independent Lantern canvas inspection | Physical input claims enter through browser events at rectangles reported by the current Rust frame; semantic actions remain for architecture/lifecycle invariants. |
...
| Lockfile identity | SHA-256 `12087febf22bb9a5ce708f39030abad61c4909fd85c8ba36957b2589d3ab4f71` |
...
| --- | ---: | ---: | --- |
| Initial loading | 0.9 ms | 21.7 ms | Thirteen samples; startup/first pipeline work dominates the tail. |
...
| Warmed idle | n/a | n/a | Frame counter stayed 232 → 232 over 700 ms; no deliberate continuous repaint. |
| Rapid zoom/tile-level transitions | 0.7 ms | 1.0 ms | The 0.1.0 p95 baseline was 126.3 ms before worker-side production and desired-set scheduling. |
| Million-row result scroll | 0.6 ms | 1.2 ms | The visible range plus eight explicit overscan rows was materialised. |
| Thumbnail gallery scroll | 0.5 ms | 0.8 ms | Five physical wheel steps advanced `0..18` to `45..63`; later decoded keys entered the bounded cache. |
| Polygon editing | 0.4 ms | 0.6 ms | Three vertices and commit across linked panes. |
| Saved workspace restore | 0.5 ms | 3.6 ms | Persisted rearranged dock restored after a full reload. |
