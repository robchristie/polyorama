...
This is a calibration foundation. Current dependency paths are provisional and
must become merged revision pins before delivery. The supported encoded profile
has one genuine quality layer; resolution discards are not additional layers.
...
One worker should own one `SharedClient` across all images/panes. Provisional
limits are 64 MiB compressed payload and 16 MiB raw plus admitted descriptor
