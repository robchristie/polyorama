...
Status: active; gallery split under qualification
...
| 2 | Split gallery story rendering into five family modules | 1 | Candidate | Focused checks and `cargo xtask verify` pass; pull request pending |

## Current phase

Review and land increment 2. The gallery shell and action registry remain in
`app.rs`; the closed `StoryId` set routes through five private story-family
modules without catalogue or baseline changes. Focused checks and the canonical
gate pass. Exact-head review and hosted CI remain before terminal closeout.
