...
export function installStartup({ app, workerUrl, scope = globalThis, timeoutMs = 60000, onFailure = () => {} }) {
  ...
  let memory, timer, raf, restoreSubmit = () => {};
  ...
  function complete() {
    if (report.status === 'starting' && report.milestones.rendering_opportunity_proxy && report.milestones.first_useful_content && (!workerUrl || report.milestones.worker_ready)) {
  ...
  function mark(name) {
    if (report.status !== 'starting' || report.milestones[name]) return false;
  ...
  scope.__polyoramaStartupWorkerMessage = data => {
    ...
    if (report.status !== 'starting') return true;
  ...
  timer = scope.setTimeout(() => fail(report.phase, `Startup exceeded ${timeoutMs / 1000} seconds`), timeoutMs);
  mark('bootstrap');
  return {
    report, mark, fail,
